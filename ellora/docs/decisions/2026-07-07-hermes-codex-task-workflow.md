...
```text
Task mới -> Hermes architect.md -> Implementation Plan -> Codex codex.md -> Implement -> Hermes review.md -> Review -> Codex Fix -> Hermes verify.md -> Done
```
...
**Impact:**
`AGENTS.md` and `docs/ai/workflows.md` now define the required workflow for future AI-assisted tasks. Trivial one-line changes can skip Hermes planning only when low-risk and called out in the final summary.

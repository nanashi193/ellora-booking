...
describe('Public salon API',()=>{
 ...
 it('maps actual DB IDs and missing images without invented ratings or availability',async()=>{
  ...
  const salon=(await result).content[0];expect(salon.id).toBe('42');expect(salon.images).toEqual(['/salon-placeholder.svg']);expect(salon.isOpen).toBeUndefined();expect(salon.reviewCount).toBe(0);
 });
